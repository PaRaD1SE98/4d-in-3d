...
def plot_3d_to_2d():
    ...
    fig = px.scatter(data_frame=df, x='x', y='y', color='z',
                    ...
                    for w in oscillate_range(20):
                        ...
                        if not r_calculated_2 <= r ** 2:
                            ...
                            data = {'x': x,
                                    ...
                                    'r': r}
                            df4d = pd.concat(
                                [df4d, pd.DataFrame(data, index=[0])])
    fig = px.scatter_3d(df4d, x='x', y='y', z='z', color='w',
                        color_continuous_scale=plotly.colors.sequential.Viridis)
...
def matrix_plot_3d_to_2d():
    ...
    fig.show()
    fig.write_html('./3dto2d_matrix.html',
...
def matrix_plot_4d_to_3d():
    ...
    r = np.sqrt((x - 0) ** 2 + (y - 0) ** 2 + (z - 0) ** 2 + (w - 5) ** 2)
    r_idx = np.where(r <= 5)
    ...
    fig.update_traces(marker=dict(size=2))
    fig.show()
    fig.write_html('./4dto3d_matrix.html',
                   include_plotlyjs='cdn', full_html=False)
...
if __name__ == '__main__':
    ...
    print(timeit.timeit(stmt=matrix_plot_4d_to_3d, number=1))
